...
- **Evaluations.** Release run on commit `a868373`, model `claude-sonnet-5`, isolated from local settings. **Routing:** 98.2% exact and 100% acceptable on 111 cases, with our 15 skills listed among 14 overlapping distractor skills; 19 of 19 held-out cases correct; no distractor steals or over-triggers. **Behavior** (8 scenarios × 3 runs, with vs without the skill): 98.4% vs 68.2% of assertions pass (+30.2 pp, 95% CI +21.5 to +38.8), but most of that gain is the suite's own conventions (IDs, statuses, register columns). On the method traps that matter most the gain is +5.6 pp (95% CI −2.2 to +15.6), which is **not yet shown**: a strong model without the skill already catches 92% of those traps. Clearest method gains: employee stages derived from the employee's progress rather than the HR workflow, and ticket counts not read as customer counts in prioritization. Known regression: with the audit skill, the model missed NPS used as a stage metric in 2 of 3 runs. Method, cost, and limits: [`docs/evaluations.md`](docs/evaluations.md). Raw results: [`evals/results/`](evals/results/).
