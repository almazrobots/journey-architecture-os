# Experience Architecture operating model

## Work products by phase

| Phase | Primary question | Primary artifact | Typical skill |
|---|---|---|---|
| Frame | What decision and scope? | Engagement brief | experience-architecture / journey-architecture |
| Evidence | What do we actually know? | Evidence register | journey-research |
| Model | What does the actor experience? | CJM/EJM | customer-journey-mapping / employee-journey-mapping |
| Explain | What produces that experience? | Service blueprint | service-blueprinting |
| Diagnose | What matters and why? | Moments + root causes | moments-that-matter |
| Measure | How will we know? | Metric tree | journey-metrics |
| Prioritize | Where should we intervene? | Opportunity register | experience-opportunity-prioritization |
| Design | What should future experience be? | Target journey | target-experience-design |
| Activate | What changes now? | Initiative/experiment links | target-experience-design |
| Govern | How does this stay alive? | Governance record | journey-governance |
| Scale | How do journeys coexist? | Journey atlas | journey-portfolio-management |

## Minimum viable journey system

For a small team, the minimum viable system is:

- one stable journey ID;
- one current-state map;
- evidence references;
- one owner;
- 3–7 useful outcome/diagnostic metrics;
- opportunity backlog;
- review date.

Scale structure only when coordination cost justifies it.
