...
- Experience (optional register): each (`node_id`, `row_type`, `text`) appears once; `node_id` exists; `valence` (−2…2) is filled on `emotion` rows and empty on all others; the observed/inferred evidence rules apply; an `observed` or `inferred` emotion must cite at least one evidence item from an interview, observation, diary, survey, or usability test.
